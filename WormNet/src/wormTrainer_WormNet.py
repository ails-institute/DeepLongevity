...
channels = 1
startingLeraningRate = 5e-4 #0.00005, 0.00001
earlyStopFlag = False
...
modelDir = "R:\\Data\\180321_DeepLongevity\\revision"
...
trainFolder = modelDir+"\\data\\bg_imgs\\train"
validateFolder = modelDir+"\\data\\bg_imgs\\validate"
testFolder = modelDir+"\\data\\bg_imgs\\test"
...
model = WormNet_200130.getWormNet()
...
reduce_lr = ReduceLROnPlateau(monitor='val_loss', factor=0.5, patience=40, min_lr=0.000001, verbose=1)
earlyStop = EarlyStopping(monitor='val_loss', min_delta=0.0001, patience=2000, verbose=1, mode='auto')
modelCheckpoint = ModelCheckpoint(os.path.join(checkpointDir,"weights.{epoch:02d}-{val_loss:.2f}.hdf5"), monitor='val_loss', verbose=1, save_best_only=False, save_weights_only=False, mode='auto', period=25)
