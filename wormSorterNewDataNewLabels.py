...
def sortImages(df, rootDir, fraction):
    for i,iPath in enumerate(range(len(df['2_cumulative_eggs_cat']))):
        print(i)
        ...
        dst = os.path.join(rootDir, fraction, df['2_cumulative_eggs_cat'].iloc[iPath], '{}_'.format(i)+df['fileNewData'].iloc[iPath].replace(" ", "__"))
        print("copy:\n"+src+"\n to:\n "+dst)
        copyfile(src, dst)
...
modelDir = ""
dataDir = "F:\\data\\day3splited_eggs"
checkDir(dataDir)
...
classNames = dataTable['2_cumulative_eggs_cat'].unique().tolist()

makeClassSubdir(trainDir,classNames)
makeClassSubdir(validateDir,classNames)
makeClassSubdir(testDir,classNames)
# Split data table
longLength = len(dataTable.loc[dataTable['2_cumulative_eggs_cat'] == 'high', '2_cumulative_eggs_cat'])
#mediumLength = len(dataTable.loc[dataTable['lifeSpanDuration'] == 'medium', 'lifeSpanHours'])
shortLength = len(dataTable.loc[dataTable['2_cumulative_eggs_cat'] == 'low', '2_cumulative_eggs_cat'])

longData = dataTable.loc[dataTable['2_cumulative_eggs_cat'] == 'high', ('2_cumulative_eggs_cat', 'pathNewData', 'fileNewData') ]
#mediumData = dataTable.loc[dataTable['lifeSpanDuration'] == 'medium', ('lifeSpanDuration', 'path', 'file') ]
shortData = dataTable.loc[dataTable['2_cumulative_eggs_cat'] == 'low', ('2_cumulative_eggs_cat', 'pathNewData', 'fileNewData') ]
