...
channels = 3
startingLeraningRate = 1e-5 #0.00005, 0.00001
earlyStopFlag = False
...
modelFile = modelDir+"\\"+"{}_worm_ResNet_fullres.json".format(runningTime)
weightsFile = modelDir+"\\"+"{}_worm_ResNet_fullres.h5".format(runningTime)

# load images

trainFolder = "F:\\data\\day3splited_newdata\\train"
validateFolder = "F:\\data\\day3splited_newdata\\validate"
testFolder = "F:\\data\\day3splited_newdata\\test"
...
for layer in InceptionV3_notop.layers[:fineTuneAt]:
...
##output = Dropout(0.5)(output)
#    #if not useRegularizer :
#    #    output = Dense(1024, activation='relu', name = 'Dense_1')(output)
#    #else :
#    #output = Dense(1024, activation='relu', name = 'Dense_1',
#     #             kernel_regularizer=regularizers.l2(0.01),
#     #             activity_regularizer=regularizers.l1(0.01))(output)
#    
#    #output = Dense(numOfClasses, activation='softmax', name='predictions',  kernel_regularizer=regularizers.l1(0.01))(output)
#output = Dense(numOfClasses, activation='softmax', name='predictions', kernel_constraint=max_norm(2.))(output)
#    
...
reduce_lr = ReduceLROnPlateau(monitor='val_loss', factor=0.5, patience=20, min_lr=0.000001, verbose=1)
